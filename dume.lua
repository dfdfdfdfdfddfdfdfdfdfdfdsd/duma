...
pcall(function()
    ...
    if not statusGui.Parent then
        ...
        statusGui.Parent = lp:FindFirstChildOfClass("PlayerGui") or lp:WaitForChild("PlayerGui", 5)
    end
end)

local sf = Instance.new("Frame")
sf.Size = UDim2.new(0, 230, 0, 40)
sf.AnchorPoint = Vector2.new(1, 1)
sf.Position = UDim2.new(1, -20, 1, -60)
sf.BackgroundColor3 = Color3.fromRGB(15, 15, 18)
...
sfStroke.Color = Color3.fromRGB(210, 185, 120)
sfStroke.Thickness = 1
sfStroke.Transparency = 0.5
sfStroke.Parent = sf

local sIcon = Instance.new("ImageLabel")
sIcon.Size = UDim2.new(0, 26, 0, 26)
sIcon.Position = UDim2.new(0, 7, 0.5, -13)
sIcon.BackgroundTransparency = 1
sIcon.Image = "rbxassetid://134841272452705"
sIcon.Parent = sf

local sText = Instance.new("TextLabel")
sText.Size = UDim2.new(1, -45, 1, 0)
sText.Position = UDim2.new(0, 40, 0, 0)
sText.BackgroundTransparency = 1
sText.Text = "Status : Kaitun"
...
task.spawn(function()
    while true do
        task.wait(0.5)
        if sText.Parent then
            local p = _G.KaitunPhase or "Idle"
            if _G.TargetMob then
                p = _G.TargetMob.Name
            end
            sText.Text = "Status : " .. tostring(p)
        end
    end
end)
...
task.spawn(function()
    ...
    for i, v in ipairs(lightingDescendants) do
...
_G.LastSummonTime = 0
_G.LastStrongestBossSummonTime = 0
...
local ConfigFolder = "DieverHub_Data"
...
local ExternalConfig = (type(getgenv) == "function" and getgenv().Config) or _G.Config or {}
local MoMoConfig = {}
for k, v in pairs(DefaultConfig) do MoMoConfig[k] = v end
...
if SavedData then
    ...
    if ExternalConfig.AutoLoad ~= nil then
        shouldAutoLoad = ExternalConfig.AutoLoad
    else
        shouldAutoLoad = SavedData.AutoLoad
    end
    ...
    if shouldAutoLoad then
        ...
        if SavedData.AutoLoad ~= nil then MoMoConfig.AutoLoad = SavedData.AutoLoad end
        if SavedData.AutoSave ~= nil then MoMoConfig.AutoSave = SavedData.AutoSave end
    end
end

for k, v in pairs(ExternalConfig) do
    MoMoConfig[k] = v
end
...
local Window = {
    Notify = function(self, tbl)
        print("[DieverHub] " .. tostring(tbl.Title) .. " - " .. tostring(tbl.Desc))
    end
}
...
Window:Notify({Title = "DieverHub", Desc = "God Mode Loaded!", Time = 4})
